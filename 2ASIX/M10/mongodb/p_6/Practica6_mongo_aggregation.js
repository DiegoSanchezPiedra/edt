...
db.sales.aggregate(
   [
      {
        $group : {
           _id : { month: { $month: "$date" }, day: { $dayOfMonth: "$date" }, year: { $year: "$date" } }, //es per indicar quins camps agruparem (en aquest cas els agruparem per mes, dia i any, és a dir que sortiràn grups de 3)
           totalPrice: { $sum: { $multiply: [ "$price", "$quantity" ] } },//creem aquest camp, de cada registre de grup farà la multiplicació de $price i $quantity, i aquest resultat el sumarà amb els resultats de cada registre del grup que s'ha fet
...
db.sales.aggregate(
   [
      {
        $group : {
           _id : null, //agrupa a tots el registres
...
db.books.aggregate(
   [
     { $group : { _id : "$author", books: { $push: "$title" } } } //agrupa per autor (homer i dante) i amb el $push posa en array tots els llibres (tittle) que hi ha per cada autor
   ]
)
...
db.books.aggregate(
   [
     { $group : { _id : "$author", books: { $push: "$$ROOT" } } } //idem que l'anterior pero crea una llista de registres partir des de el root
   ]
)
